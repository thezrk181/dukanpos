...
function App() {
  ...
  return <WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/, '')}><Shell><Switch><Route path="/" component={() => <Counter state={state} setState={setState} notify={notify} />} /><Route path="/inventory" component={() => <Inventory state={state} setState={setState} notify={notify} />} /><Route path="/udhaar" component={() => <Udhaar state={state} setState={setState} notify={notify} />} /><Route path="/summary" component={() => <Summary state={state} />} /><Route component={NotFound} /></Switch></Shell>{toast && <div data-testid="status-toast" className="no-print fixed bottom-20 left-1/2 z-[60] -translate-x-1/2 rounded-xl bg-primary px-4 py-3 text-xs font-bold text-primary-foreground shadow-xl md:bottom-6">{toast}</div>}</WouterRouter>;
}
export default App;