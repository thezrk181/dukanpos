...
function loadState(): State {
  try {
    ...
    return {
      products: (saved.products ?? seed.products).map((p) => ({ ...p, barcode: p.barcode || barcodes.get(p.id), sellBy: p.sellBy || 'unit' })),
...
function SettingsModal({ session, onClose }: { session: any; onClose: () => void }) {
  const [ownerName, setOwnerName] = useState(session?.user?.user_metadata?.ownerName || '');
  const [storeName, setStoreName] = useState(session?.user?.user_metadata?.storeName || '');
  const [loading, setLoading] = useState(false);
  
  const save = async () => {
    setLoading(true);
    await supabase.auth.updateUser({ data: { ownerName, storeName } });
    setLoading(false);
    onClose();
  };
  ...
  return <div className="no-print fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm transition-all animate-in fade-in">
    <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-2xl animate-in zoom-in-95">
      <div className="flex items-center justify-between border-b border-border pb-4 mb-5">
        <h2 className="font-display text-xl text-foreground">Shop Settings</h2>
        <button onClick={onClose} className="rounded-lg p-2 hover:bg-muted text-muted-foreground"><X size={18} /></button>
      </div>
      <div className="space-y-4">
        <label className="block text-xs font-bold text-muted-foreground">Your Name
          <input value={ownerName} onChange={(e) => setOwnerName(e.target.value)} placeholder="e.g. Aamir Khan" className="mt-1.5 h-11 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground shadow-sm" />
        </label>
        <label className="block text-xs font-bold text-muted-foreground">Store Name
          <input value={storeName} onChange={(e) => setStoreName(e.target.value)} placeholder="e.g. Al-Khan General Store" className="mt-1.5 h-11 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground shadow-sm" />
        </label>
        <button onClick={save} disabled={loading} className="w-full rounded-xl bg-primary py-3 text-xs font-bold text-primary-foreground shadow-md transition hover:bg-primary/90">{loading ? 'Saving...' : 'Save Changes'}</button>
        <div className="border-t border-border pt-5 mt-2">
          <button onClick={logout} className="w-full flex items-center justify-center gap-2 rounded-xl border border-destructive/30 bg-destructive/10 py-3 text-xs font-bold text-destructive hover:bg-destructive hover:text-white transition shadow-sm"><Zap size={16} /> Sign Out securely</button>
        </div>
      </div>
    </div>
  </div>;
...
function Shell({ children, session }: { children: ReactNode; session: any }) {
  ...
  const ownerName = session?.user?.user_metadata?.ownerName || 'Shop Owner';
  const storeName = session?.user?.user_metadata?.storeName || 'My Shop';
  const initls = initials(ownerName);
  ...
  return <div className="min-h-[100dvh] bg-background">
    <aside className={`no-print fixed inset-y-0 left-0 z-40 w-[248px] bg-sidebar px-4 py-5 text-sidebar-foreground transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="absolute bottom-5 left-6 right-6 flex items-center gap-3 border-t border-sidebar-border pt-4"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#dbe6dd] text-xs font-bold text-primary">{initls}</div><div className="min-w-0"><p className="truncate text-xs font-bold">{ownerName}</p><p className="truncate text-[10px] text-[#91a0ac]">{storeName}</p></div><button onClick={() => setSettingsOpen(true)} className="ml-auto shrink-0 p-1 text-[#8e9aa5] hover:text-[#f4c95d] transition"><Settings2 size={16} /></button></div>
    </aside>
    ...
    {settingsOpen && <SettingsModal session={session} onClose={() => setSettingsOpen(false)} />}
...
function Counter({ state, setState, notify, session }: { state: State; setState: Dispatch<SetStateAction<State>>; notify: (s: string) => void; session: any }) {
  const [query, setQuery] = useState(''); const [category, setCategory] = useState('All'); const [cart, setCart] = useState<CartItem[]>([]); const [checkout, setCheckout] = useState<'cash' | 'udhaar' | null>(null); const [customer, setCustomer] = useState(''); const [paid, setPaid] = useState(''); const [receipt, setReceipt] = useState<Sale | null>(null); const [scanner, setScanner] = useState(false); const [promptQty, setPromptQty] = useState<{product: Product, currentQty: string} | null>(null);
  ...
  return <><Topbar eyebrow={dateLabel()} title={`Good ${new Date().getHours() < 12 ? 'morning' : 'afternoon'}, ${session?.user?.user_metadata?.ownerName?.split(' ')[0] || 'there'}`} action={<button data-testid="button-scan" onClick={() => setScanner(true)} className="mt-1 flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-sm font-bold text-primary shadow-sm hover:bg-muted"><ScanBarcode size={17} /> <span className="hidden sm:inline">Scan item</span></button>} />
    ...
    {promptQty && <Modal title="Enter quantity" onClose={() => setPromptQty(null)}>
      <div className="space-y-4">
        <label className="block text-sm font-bold">Weight / Volume ({promptQty.product.unit})
          <input autoFocus type="number" step="0.01" value={promptQty.currentQty} onChange={(e) => setPromptQty({ ...promptQty, currentQty: e.target.value })} placeholder="e.g. 1.5" className="mt-2 h-12 w-full rounded-xl border border-border bg-card px-4 font-mono text-lg" />
        </label>
...
function Inventory({ state, setState, notify }: { state: State; setState: Dispatch<SetStateAction<State>>; notify: (s: string) => void }) {
  const [search, setSearch] = useState(''); const [filter, setFilter] = useState<'all' | 'low'>('all'); const [categoryFilter, setCategoryFilter] = useState('All'); const [modal, setModal] = useState<Product | 'new' | null>(null); const [scanner, setScanner] = useState(false); const [form, setForm] = useState<{ name: string; category: string; price: string; cost: string; stock: string; unit: string; barcode: string; sellBy: 'unit' | 'weight' }>({ name: '', category: 'Kiryana', price: '', cost: '', stock: '', unit: 'pack', barcode: '', sellBy: 'unit' });
  const categories = ['All', ...Array.from(new Set(state.products.map((p) => p.category)))];
  const list = state.products.filter((p) => {
    const matchesSearch = p.name.toLowerCase().includes(search.toLowerCase()) || p.barcode?.includes(search.trim());
    const matchesFilter = filter === 'low' ? p.stock < 6 : true;
    const matchesCategory = categoryFilter === 'All' || p.category === categoryFilter;
    return matchesSearch && matchesFilter && matchesCategory;
  });
  const edit = (p: Product) => { setForm({ name: p.name, category: p.category, price: String(p.price), cost: String(p.cost), stock: String(p.stock), unit: p.unit, barcode: p.barcode || '', sellBy: p.sellBy || 'unit' }); setModal(p); };
  const save = () => { const current = modal; if (!current) return; if (!form.name || !form.price) return notify('Add a product name and selling price'); const duplicate = state.products.some((item) => item.barcode && form.barcode.trim() && item.barcode === form.barcode.trim() && (current === 'new' || item.id !== current.id)); if (duplicate) return notify('That barcode is already assigned'); const p: Product = { id: current === 'new' ? `p-${Date.now()}` : current.id, name: form.name.trim(), category: form.category, price: Number(form.price), cost: Number(form.cost) || 0, stock: Math.max(0, Number(form.stock) || 0), unit: form.unit.trim() || 'pack', barcode: form.barcode.trim() || undefined, sellBy: form.sellBy, accent: current === 'new' ? 'bg-[#dbe6dd]' : current.accent }; setState((s) => ({ ...s, products: current === 'new' ? [p, ...s.products] : s.products.map((x) => x.id === p.id ? p : x) })); setModal(null); notify(current === 'new' ? 'Product added to shelf' : 'Product updated'); };
  ...
  return <><Topbar eyebrow="Stock room" title="Inventory" action={<button data-testid="button-add-product" onClick={() => { setForm({ name: '', category: 'Kiryana', price: '', cost: '', stock: '', unit: 'pack', barcode: '', sellBy: 'unit' }); setModal('new'); }} className="mt-1 flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:brightness-110"><Plus size={17} /> Add product</button>} /><div className="p-5 pb-24 md:p-9"><div className="mb-5 grid gap-3 sm:grid-cols-3"><Stat icon={Package} label="Total products" value={String(state.products.length)} tint="teal" active={filter === 'all'} onClick={() => setFilter('all')} /><Stat icon={ArrowDownLeft} label="Low stock" value={String(state.products.filter((p) => p.stock < 6).length)} tint="coral" active={filter === 'low'} onClick={() => setFilter('low')} /><Stat icon={CircleDollarSign} label="Shelf value" value={money(state.products.reduce((a, p) => a + p.cost * p.stock, 0))} tint="yellow" /></div><div className="rounded-2xl border border-border bg-card shadow-sm"><div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-2xl">All products</h2><p className="text-xs text-muted-foreground">Keep your shelf count honest.</p></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><select data-testid="select-inventory-category" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-xs">{categories.map(c => <option key={c} value={c}>{c}</option>)}</select><div className="relative sm:w-64"><Search size={16} className="absolute left-3 top-3 text-muted-foreground" /><input data-testid="input-inventory-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Find by name or barcode..." className="h-10 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-xs" /></div></div></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead className="bg-muted/60 text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-5 py-3 font-bold">Product</th><th className="px-4 py-3 font-bold">Barcode</th><th className="px-4 py-3 font-bold">Category</th><th className="px-4 py-3 font-bold">Selling price</th><th className="px-4 py-3 font-bold">In stock</th><th className="px-4 py-3" /></tr></thead><tbody>{list.map((p) => <tr key={p.id} data-testid={`row-inventory-${p.id}`} className="border-t border-border/70"><td className="px-5 py-3"><div className="flex items-center gap-3"><div className={`grid h-9 w-9 place-items-center rounded-lg ${p.accent}`}><Package size={16} className="text-[#243348]/40" /></div><div><p className="text-xs font-bold">{p.name}</p><p className="font-mono text-[10px] text-muted-foreground">Cost {money(p.cost)}</p></div></div></td><td className="px-4 font-mono text-[10px] text-muted-foreground">{p.barcode || '—'}</td><td className="px-4 text-xs text-muted-foreground">{p.category}</td><td className="px-4 font-mono text-xs font-bold text-primary">{money(p.price)}</td><td className="px-4"><span className={`rounded-md px-2 py-1 font-mono text-[10px] font-bold ${p.stock < 6 ? 'bg-accent/10 text-accent' : 'bg-secondary text-primary'}`}>{p.stock} {p.unit}s</span></td><td className="px-4 text-right"><button data-testid={`button-edit-product-${p.id}`} onClick={() => edit(p)} className="mr-1 rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-primary"><Settings2 size={15} /></button><button data-testid={`button-delete-product-${p.id}`} onClick={() => del(p.id)} className="rounded-lg p-2 text-muted-foreground hover:bg-accent/10 hover:text-destructive"><Trash2 size={15} /></button></td></tr>)}</tbody></table></div>{!list.length && <Empty icon={Package} title="Shelf is clear" text="No products match this search." />}</div></div>{modal && <Modal title={modal === 'new' ? 'Add product' : 'Edit product'} onClose={() => setModal(null)}><div className="grid gap-3"><label className="text-xs font-bold">Product name<input data-testid="input-product-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border px-3 text-sm" placeholder="e.g. Shan Masala 100g" /></label><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold">Category<select data-testid="select-product-category" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border bg-card px-3 text-sm"><option>Kiryana</option><option>Dairy</option><option>Snacks</option><option>Household</option><option>Drinks</option></select></label><label className="text-xs font-bold">Selling method<select data-testid="select-product-sellby" value={form.sellBy} onChange={(e) => setForm({ ...form, sellBy: e.target.value as 'unit' | 'weight' })} className="mt-1 h-11 w-full rounded-lg border border-border bg-card px-3 text-sm"><option value="unit">Fixed unit (e.g. bags)</option><option value="weight">Loose / Weight (e.g. kg)</option></select></label></div><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold">Unit label (e.g. kg)<input data-testid="input-product-unit" value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border px-3 text-sm" /></label><label className="text-xs font-bold">Stock<input data-testid="input-product-stock" type="number" step={form.sellBy === 'weight' ? '0.01' : '1'} value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border px-3 font-mono" /></label></div><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold">Cost<input data-testid="input-product-cost" type="number" value={form.cost} onChange={(e) => setForm({ ...form, cost: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border px-3 font-mono" /></label><label className="text-xs font-bold">Sell price<input data-testid="input-product-price" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="mt-1 h-11 w-full rounded-lg border border-border px-3 font-mono" /></label></div><label className="text-xs font-bold block">Barcode<div className="mt-1 flex gap-2"><input data-testid="input-product-barcode" value={form.barcode} onChange={(e) => setForm({ ...form, barcode: e.target.value })} placeholder="Optional" className="h-11 flex-1 rounded-lg border border-border px-3 font-mono" /><button type="button" onClick={() => setScanner(true)} className="flex h-11 items-center justify-center rounded-lg border border-border bg-background px-4 text-xs font-bold text-muted-foreground hover:bg-muted hover:text-primary"><ScanBarcode size={16} className="mr-2" />Scan</button></div></label><button data-testid="button-save-product" onClick={save} className="mt-2 rounded-xl bg-primary py-3 text-xs font-bold text-primary-foreground">Save product</button></div></Modal>}{scanner && <ScannerModal onCode={(code) => { setForm({ ...form, barcode: code }); setScanner(false); }} onClose={() => setScanner(false)} />}</>;
}

function Stat({ icon: Icon, label, value, tint, active, onClick }: { icon: typeof Package; label: string; value: string; tint: string; active?: boolean; onClick?: () => void }) { return <div onClick={onClick} className={`rounded-2xl border ${active ? 'border-primary ring-1 ring-primary' : 'border-border'} bg-card p-4 shadow-sm ${onClick ? 'cursor-pointer transition hover:-translate-y-0.5 hover:shadow-counter' : ''}`}><div className="flex items-center justify-between"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tint === 'coral' ? 'bg-accent/10 text-accent' : tint === 'yellow' ? 'bg-[#f4c95d]/25 text-[#9d7010]' : 'bg-secondary text-primary'}`}><Icon size={17} /></span><ArrowUpRight size={15} className="text-muted-foreground" /></div><p data-testid={`text-stat-${label.replaceAll(' ', '-')}`} className="mt-4 font-mono text-xl font-bold">{value}</p><p className="mt-1 text-xs text-muted-foreground">{label}</p></div>; }
...
function Summary({ state, session }: { state: State; session: any }) {
  ...
  return <><Topbar eyebrow="End of day" title="Today at a glance" action={<button data-testid="button-print-summary" onClick={() => window.print()} className="mt-1 flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-sm font-bold text-primary"><Printer size={17} /> <span className="hidden sm:inline">Print summary</span></button>} /><div className="p-5 pb-24 md:p-9 print:hidden"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl bg-primary p-5 text-primary-foreground shadow-counter sm:col-span-2"><p className="text-xs font-semibold opacity-75">Sales today</p><p data-testid="text-summary-sales" className="mt-2 font-mono text-4xl font-bold">{money(revenue)}</p><div className="mt-6 flex gap-8 text-xs"><span><b className="block text-lg font-mono">{sales.length}</b>transactions</span><span><b className="block text-lg font-mono">{items}</b>items sold</span></div></div><Stat icon={Banknote} label="Cash received" value={money(cash)} tint="yellow" /><Stat icon={CreditCard} label="Added to udhaar" value={money(udhaar)} tint="coral" /></div><div className="mt-7 grid gap-5 xl:grid-cols-[1.25fr_.75fr]"><section className="min-w-0 rounded-2xl border border-border bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border p-5"><div><h2 className="font-display text-2xl">Recent sales</h2><p className="text-xs text-muted-foreground">Select a sale to view or print its bill.</p></div><span className="rounded-lg bg-secondary px-2.5 py-1 font-mono text-[10px] font-bold text-primary">{sales.length} TOTAL</span></div>{sales.length ? <div className="divide-y divide-border/70">{sales.slice(0, 8).map((s) => <button data-testid={`row-sale-${s.id}`} key={s.id} onClick={() => setReceipt(s)} className="flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-muted/50"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${s.type === 'cash' ? 'bg-secondary text-primary' : 'bg-accent/10 text-accent'}`}>{s.type === 'cash' ? <Banknote size={16} /> : <CreditCard size={16} />}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{s.items.map((i) => i.name).join(', ')}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">{new Date(s.createdAt).toLocaleTimeString('en-PK', { hour: '2-digit', minute: '2-digit' })} · {s.type === 'cash' ? 'Cash sale' : 'Udhaar'}</p></div><span className="font-mono text-sm font-bold shrink-0">{money(s.total)}</span><Receipt size={15} className="shrink-0 text-muted-foreground" /></button>)}</div> : <Empty icon={FileText} title="No sales yet today" text="Your first sale will show up here." />}</section><section className="min-w-0 rounded-2xl border border-border bg-card p-5 shadow-sm"><h2 className="font-display text-2xl">Shelf watch</h2><p className="text-xs text-muted-foreground">Restock before the rush.</p><div className="mt-5 space-y-4">{state.products.filter((p) => p.stock < 10).sort((a, b) => a.stock - b.stock).map((p) => <div key={p.id} className="flex items-center gap-3"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ${p.accent}`}><Package size={15} className="text-[#243348]/40" /></div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{p.name}</p><div className="mt-1 h-1.5 rounded-full bg-muted"><div className={`h-1.5 rounded-full ${p.stock < 5 ? 'bg-accent' : 'bg-[#d9ad3e]'}`} style={{ width: `${Math.min(p.stock * 10, 100)}%` }} /></div></div><span className="font-mono text-xs font-bold text-accent shrink-0">{p.stock}</span></div>)}{!state.products.some((p) => p.stock < 10) && <p className="py-8 text-center text-sm text-muted-foreground">Everything is well stocked.</p>}</div></section></div></div>
  <div className="hidden print:block print-receipt max-w-sm mx-auto mt-8 font-mono text-xs">
    <div className="text-center"><div className="font-display text-2xl text-primary">dukan</div><p className="mt-1 text-[10px] text-muted-foreground">{storeName}</p><p className="mt-1 text-[10px] text-muted-foreground">End of Day: {new Date().toLocaleDateString('en-PK')}</p></div>
    <div className="my-4 border-t border-dashed border-border" />
    <div className="flex justify-between font-bold text-sm"><span>TOTAL REVENUE</span><span>{money(revenue)}</span></div>
    <div className="mt-2 flex justify-between text-muted-foreground"><span>Cash Received</span><span>{money(cash)}</span></div>
    <div className="flex justify-between text-muted-foreground"><span>Udhaar Added</span><span>{money(udhaar)}</span></div>
    <div className="my-4 border-t border-dashed border-border" />
    <p className="font-bold mb-2">Transactions ({sales.length})</p>
    {sales.length === 0 && <p className="text-muted-foreground text-center my-4">No sales today</p>}
    {sales.map(s => <div key={s.id} className="flex justify-between py-1 border-b border-dashed border-border/50 last:border-0"><span className="truncate pr-2">{new Date(s.createdAt).toLocaleTimeString('en-PK', { hour: '2-digit', minute: '2-digit' })} ({s.type})</span><span>{money(s.total)}</span></div>)}
    <div className="my-4 border-t border-dashed border-border" />
...
function App() {
  ...
  return <WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/, '')}><Shell session={session}><Switch><Route path="/"><Counter state={state} setState={setState} notify={notify} session={session} /></Route><Route path="/inventory"><Inventory state={state} setState={setState} notify={notify} /></Route><Route path="/udhaar"><Udhaar state={state} setState={setState} notify={notify} /></Route><Route path="/summary"><Summary state={state} session={session} /></Route><Route><NotFound /></Route></Switch></Shell>{toast && <div data-testid="status-toast" className="no-print fixed bottom-20 left-1/2 z-[60] -translate-x-1/2 rounded-xl bg-primary px-4 py-3 text-xs font-bold text-primary-foreground shadow-xl md:bottom-6">{toast}</div>}</WouterRouter>;
}
export default App;