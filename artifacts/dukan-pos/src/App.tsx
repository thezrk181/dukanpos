...
function Inventory({ state, setState, notify }: { state: State; setState: Dispatch<SetStateAction<State>>; notify: (s: string) => void }) {
  const [search, setSearch] = useState(''); const [filter, setFilter] = useState<'all' | 'low'>('all'); const [categoryFilter, setCategoryFilter] = useState('All'); const [modal, setModal] = useState<Product | 'new' | null>(null); const [form, setForm] = useState<{ name: string; category: string; price: string; cost: string; stock: string; unit: string; barcode: string; sellBy: 'unit' | 'weight' }>({ name: '', category: 'Kiryana', price: '', cost: '', stock: '', unit: 'pack', barcode: '', sellBy: 'unit' });
  const categories = ['All', ...Array.from(new Set(state.products.map((p) => p.category)))];
